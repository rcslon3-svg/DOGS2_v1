 ...
 *   UI_BUTTON_GPIO - external UI button
 ...
 *   last_raw/stable/count/pressed_previous - debouncer storage for one button.
 * Returns: true once, on a new confirmed press.
 * Does: accepts a changed raw level only after it is seen for several calls.
 */
                             ...
                             bool *pressed_previous)
{
    ...
    if (*count >= BUTTON_DEBOUNCE_TICKS) *stable = raw_high;
...
{
    s_mode = (app_mode_t)(((uint8_t)s_mode + 1U) % (uint8_t)APP_MODE_COUNT);
    clear_selection();
}
...
void control_update(app_state_t *state)
{
    ...
    if (ui_pressed || encoder_button_pressed) {
        button_advance_selection();
        mark_control_activity(now_us);
    }
